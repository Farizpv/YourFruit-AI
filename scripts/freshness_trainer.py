...
def train_freshness_model(fruit_name, batch_size=32, img_size=(150,150), initial_epochs=10, fine_tune_epochs=20):
    ...
    # Data generators with aggressive augmentation for train
    train_datagen = ImageDataGenerator(
        preprocessing_function=preprocess_input, # <--- USE THIS INSTEAD OF RESCALE
    ...
    test_datagen = ImageDataGenerator(preprocessing_function=preprocess_input) # <--- USE THIS INSTEAD OF RESCALE
    ...
    if train_generator.num_classes < 2:
        print(f"⚠️ Skipping {fruit_name}: Not enough freshness categories (found {train_generator.num_classes}). Need at least 2.")
        return # Skip training if only one class is found

    num_classes = train_generator.num_classes

    # Build model with MobileNetV2 base (frozen first)
    ...
    # Use initial_epoch=0 for the first fit call
    history = model.fit(
        train_generator,
    ...
    model.fit(
        ...
        initial_epoch=history.epoch[-1], # Start from where initial training left off
        callbacks=[early_stop, reduce_lr]
    )
...
if __name__ == "__main__":
    ...
    for i, fruit in enumerate(common_fruits, 1):
        ...
        print(f"⏱️ Time taken for {fruit}: {elapsed:.2f} seconds")
